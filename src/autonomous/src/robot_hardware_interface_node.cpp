#include <autonomous/robot_hardware_interface.h>
...
void ROBOTHardwareInterface::read() {
    ...
    left_front_motor.readBytes(rbuff,1);
    x=(int8_t)rbuff[0];
    left_front_motor_pos+=angles::from_degrees((double)x);
    joint_position_[0]=left_front_motor_pos;

    left_back_motor.readBytes(rbuff,1);
    x=(int8_t)rbuff[0];
    left_back_motor_pos+=angles::from_degrees((double)x);
    joint_position_[0]=left_back_motor_pos;
    
    right_front_motor.readBytes(rbuff,1);
    x=(int8_t)rbuff[0];
    right_front_motor_pos+=angles::from_degrees((double)x);
    joint_position_[1]=right_front_motor_pos;

    right_back_motor.readBytes(rbuff,1);
    x=(int8_t)rbuff[0];
    right_back_motor_pos+=angles::from_degrees((double)x);
    joint_position_[1]=right_back_motor_pos;
...
void ROBOTHardwareInterface::write(ros::Duration elapsed_time) {
    ...
    if(left_front_prev_cmd!=velocity)
    {
	    result = left_front_motor.writeData(wbuff,2);
	    //ROS_INFO("Writen successfully result=%d", result);
	    left_front_prev_cmd=velocity;
    }
    
    if(left_back_prev_cmd!=velocity)
    {
	    result = left_back_motor.writeData(wbuff,2);
	    //ROS_INFO("Writen successfully result=%d", result);
	    left_back_prev_cmd=velocity;
    }
    ...
    if(right_front_prev_cmd!=velocity)
    {
	    result = right_front_motor.writeData(wbuff,2);
	    //ROS_INFO("Writen successfully result=%d", result);
	    right_front_prev_cmd=velocity;
    }
    
    if(right_back_prev_cmd!=velocity)
    {
	    result = right_back_motor.writeData(wbuff,2);
	    //ROS_INFO("Writen successfully result=%d", result);
	    right_back_prev_cmd=velocity;
    }
